...
/* ─── Inline Edit Form ─────────────────────────────── */
export default function ClientActions({ client, editAction, deleteAction }) {
  ...
  async function handleEdit(e) {
    e.preventDefault();
    if (!name.trim() || name.trim() === client.name) { setEditing(false); return; }
    ...
    <>
      {editing ? (
        <form onSubmit={handleEdit} className="flex items-center gap-2 w-full sm:w-auto">
          ...
          <button
            type="button" onClick={() => { setEditing(false); setName(client.name); }}
